        ...
        <div className="h-full flex flex-col justify-between">
            <div className="mt-8 flex-1 overflow-y-auto">
                <ul className="-my-6 divide-y divide-gray-200">
                    {cartCount=== 0 ? (
                        <h1 className="text-sm py-6">You do not have any items yet.</h1>
                    ):(
                        <>
                        {Object.values(cartDetails ?? {}).map((entry)=>(
                            <li key={entry.id} className="flex py-6">
                                <div className="h-24 w-24 flex-shrink-0 overflow-hidden rounded-md border border-gray-200 	">
                                    <Image alt="photo" src={entry.image as string}
                                    width={100}
                                    height={100}
                                    />
                                </div>
                                <div className="ml-4 flex flex-1 flex-col">
                                    ...
                                    <div className="flex flex-1 items-end justify-between text-sm">
                                        <p className="text-gray-500">Items: {entry.quantity}</p>
                                        <div className="flex">
            ...
            <div className="border-t border-gray-200 px-4 py-6 sm:px-6">
                ...
                <div className="mt-6">
                    <Button onClick={(event)=>handleCheckoutClick(event)} className="w-full">Checkout</Button>
                </div>
